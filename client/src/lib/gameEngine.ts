...
export function grabChunk(state: GameState, sourceId: string): GameState {
  ...
  // Grab up to grabNumber cats — if fewer remain, grab all of them
  const grabCount = Math.min(source.grabNumber, source.stack.length);
  const grabbed = source.stack.splice(source.stack.length - grabCount, grabCount).reverse();
